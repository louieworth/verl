...
class TestDPOPairDataset(unittest.TestCase):
    def test_dataset_shapes_and_masks(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            data_path = Path(tmpdir) / "dpo.jsonl"
            row = {
            ...
            dataset = DPOPairDataset(str(data_path), tokenizer=DummyTokenizer(), config=cfg)
            sample = dataset[0]

            self.assertEqual(sample["chosen_input_ids"].shape[0], 14)
            self.assertEqual(sample["chosen_responses"].shape[0], 6)
            self.assertEqual(sample["chosen_response_mask"].sum().item(), 6)
            self.assertTrue((sample["chosen_labels"][:8] == -100).all())
