...
PASS_K=${PASS_K:-4}
TEMPERATURE=${CODE_EVAL_TEMPERATURE:-1.0}
TOP_P=${CODE_EVAL_TOP_P:-1.0}
...
if avg is not None:
    entry[f"{dataset}_avg4"] = avg
if passed is not None:
    entry[f"{dataset}_pass4"] = passed
...
for MODEL_PATH in "${MODEL_PATHS[@]}"; do
    ...
    echo "# Sampling:   pass_k=$PASS_K temperature=$TEMPERATURE top_p=$TOP_P max_prompt_tokens=$MAX_PROMPT_TOKENS max_response_tokens=$MAX_TOKENS max_model_len=$MAX_MODEL_LEN"
    echo "################################################################################"
