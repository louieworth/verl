...
#
# Usage:
#   bash run_full_pipeline_multi_epoch.sh
#
# Environment Variables:
#   STAGE2_PREPARE_SCRIPT=recipe/open_math_reasoning/stage2_prepare.py  # Options:
#     - stage2_prepare.py         # Original prompt (V1)
#     - stage2_prepare_v2.py      # Minimal-edit-first strategy (V2)
#     - stage2_prepare_v3_fewshot.py  # V2 + 3-shot examples from successful corrections (V3, recommended)
#   FORCE_RERUN=false    # Set to 'true' to disable auto-resume and rerun everything
#
# Auto-Resume Behavior (所有文件都在 gen_results/，只有 results.json 在 results/):
#   检查 gen_results/${MODEL_NAME}/epoch${EPOCH}/ 下的完成标记:
#     * Stage 1: deepscaleR_stage1.parquet + stage1_eval_results.json
#     * Stage 2: stage2_correction.parquet + stage2_eval_results.json
#     * Stage 3 SFT: deepscaleR_stage3_sft.parquet + .stage3_sft_completed
#     * Stage 3 Eval: evaluate/ 目录存在且有文件
#   results/${MODEL_NAME}/results.json 汇总所有 epoch 的测试集评估
...
#   - rm -rf gen_results/<MODEL_NAME>/epoch<N>/
#   - rm -rf /path/to/checkpoint/<MODEL_NAME>_epoch<N>/
#   注意: results/ 只包含 results.json，不需要删除 epoch 子目录
...
detect_epoch_progress() {
    ...
    # 目录结构简化:
    #   gen_results/${MODEL_NAME}/  - 所有中间文件（数据集、评估）
    #   results/${MODEL_NAME}/       - 只有 results.json 汇总

    # Stage 1
    local STAGE1_PROMPTS="gen_results/${MODEL_NAME}/epoch${EPOCH}/deepscaleR_stage1_prompts.parquet"
    local STAGE1_DATA="gen_results/${MODEL_NAME}/epoch${EPOCH}/deepscaleR_stage1_responses.parquet"
    local STAGE1_EVAL="gen_results/${MODEL_NAME}/epoch${EPOCH}/stage1_eval_results.json"

    # Stage 2
    local STAGE2_DATA="gen_results/${MODEL_NAME}/epoch${EPOCH}/stage2_correction.parquet"
    local STAGE2_EVAL="gen_results/${MODEL_NAME}/epoch${EPOCH}/stage2_eval_results.json"

    # Stage 3
    local STAGE3_SFT_DATA="gen_results/${MODEL_NAME}/epoch${EPOCH}/deepscaleR_stage3_sft.parquet"
    local STAGE3_SFT_MARKER="gen_results/${MODEL_NAME}/epoch${EPOCH}/.stage3_sft_completed"
    local STAGE3_EVAL_DIR="gen_results/${MODEL_NAME}/epoch${EPOCH}/evaluate"
    local CKPT_HOME="${CKPT_HOME_BASE}_epoch${EPOCH}"

    # 最终汇总文件（results/ 中唯一文件）
    local FINAL_RESULTS_JSON="results/${MODEL_NAME}/results.json"

    local completed_stages=()

# Check Stage 1a: Generation completion (检查生成数据文件）
    if file_exists_and_nonempty "${STAGE1_DATA}"; then
        completed_stages+=("stage1_gen")
    fi

# Check Stage 1b: Evaluation completion (检查评估结果文件）
    if file_exists_and_nonempty "${STAGE1_EVAL}"; then
        completed_stages+=("stage1_eval")
    fi

# Check if Stage 1 is fully completed
    if [[ " ${completed_stages[@]} " =~ " stage1_gen " ]] && [[ " ${completed_stages[@]} " =~ " stage1_eval " ]]; then
        completed_stages+=("stage1")
    fi
    # Check Stage 2 completion
    if file_exists_and_nonempty "${STAGE2_DATA}" && file_exists_and_nonempty "${STAGE2_EVAL}"; then
    ...
    # Check Stage 3 Evaluation completion (evaluate/ 目录存在且有文件)
    if dir_exists_and_nonempty "${STAGE3_EVAL_DIR}"; then
...
mark_stage_completed() {
    ...
    if [ "${STAGE}" == "stage3_sft" ]; then
        local STAGE3_SFT_MARKER="gen_results/${MODEL_NAME}/epoch${EPOCH}/.stage3_sft_completed"
        mkdir -p "$(dirname "${STAGE3_SFT_MARKER}")"
        touch "${STAGE3_SFT_MARKER}"
    fi
}
...
# Stage 2 preparation script (v1: default, v2: minimal-edit-first strategy)
STAGE2_PREPARE_SCRIPT=${STAGE2_PREPARE_SCRIPT:-recipe/open_math_reasoning/stage2_prepare_v2.py}
...
run_stage3_evaluation() {
    ...
    local EPOCH=$2
    local EVAL_OUTPUT_DIR="gen_results/${MODEL_NAME}/epoch${EPOCH}/evaluate"
    ...
    for CURRENT_PASS_K in ${TEST_PASS_K_VALUES}; do
        ...
        for DATASET_NAME in aime24 aime25 math500 hmmt25; do
            ...
            python3 -m verl.trainer.main_eval \
                ...
                +output_json_path="results/${MODEL_NAME}/results.json" \
...
run_epoch() {
    ...
    # Note: All intermediate files are in gen_results/${MODEL_NAME}/epoch${EPOCH}/
    # results/ only contains results.json

    # Define file paths for this epoch
    local STAGE1_PROMPTS="gen_results/${MODEL_NAME}/epoch${EPOCH}/deepscaleR_stage1_prompts.parquet"
    local STAGE1_DATA="gen_results/${MODEL_NAME}/epoch${EPOCH}/deepscaleR_stage1_responses.parquet"
    local STAGE1_EVAL="gen_results/${MODEL_NAME}/epoch${EPOCH}/stage1_eval_results.json"
    local STAGE2_DATA="gen_results/${MODEL_NAME}/epoch${EPOCH}/stage2_correction.parquet"
    ...
    if [[ " ${COMPLETED_STAGES[@]} " =~ " stage1 " ]]; then
        ...
        echo "  [prompts]: ${STAGE1_PROMPTS}"
        echo "  [responses]: ${STAGE1_DATA}"
        echo "  [eval]: ${STAGE1_EVAL}"
    else
        echo ""
        echo "========================================"
        ...
        # Check if generation step is already done
        local GEN_ALREADY_DONE=false
        if file_exists_and_nonempty "${STAGE1_DATA}"; then
            echo "[Step 1.2] Generation already done, skipping..."
            echo "Found existing: ${STAGE1_DATA}"
            GEN_ALREADY_DONE=true
        fi

        # Only run preparation if generation is not done
        if [ "${GEN_ALREADY_DONE}" = "false" ]; then
        ...
        # Step 1.3: Evaluate (always run if not already done)
        if file_exists_and_nonempty "${STAGE1_EVAL}"; then
            echo "[Step 1.3] Evaluation already done, skipping..."
            echo "Found existing: ${STAGE1_EVAL}"
        else
            echo "[Step 1.3] Evaluating responses..."
            python3 -m verl.trainer.main_eval \
                data.path="${STAGE1_DATA}" \
                custom_reward_function.path=recipe/open_math_reasoning/compute_score.py \
                custom_reward_function.name=compute_score_data_source \
                +output_json_path="${STAGE1_EVAL}" \
                +model_name="${MODEL_NAME}_epoch${EPOCH}" \
                +pass_k=${PASS_K}

            if [ $? -ne 0 ]; then
                echo "Epoch ${EPOCH} Stage 1 evaluation failed. Exiting."
                exit 1
            fi
        fi
    ...
    if [[ " ${COMPLETED_STAGES[@]} " =~ " stage2 " ]]; then
        ...
        # Check if correction generation is already done
        local STAGE2_GEN_ALREADY_DONE=false
        if file_exists_and_nonempty "${STAGE2_DATA}"; then
            echo "[Step 2.2] Correction generation already done, skipping..."
            echo "Found existing: ${STAGE2_DATA}"
            STAGE2_GEN_ALREADY_DONE=true
        fi

        if [ "${STAGE2_GEN_ALREADY_DONE}" = "false" ]; then
            # Step 2.1: Prepare correction data
            echo "[Step 2.1] Preparing Stage 2 correction data..."
            echo "  Using: ${STAGE2_PREPARE_SCRIPT}"
            python3 "${STAGE2_PREPARE_SCRIPT}" \
                --stage1_output "${STAGE1_DATA}" \
                --output_reward0 "gen_results/${MODEL_NAME}/epoch${EPOCH}/deepscaleR_stage2_reward0_correction.parquet" \
                --output_reward1 "gen_results/${MODEL_NAME}/epoch${EPOCH}/deepscaleR_stage2_reward1.parquet"
            ...
            python3 -m verl.trainer.main_generation_server \
                ...
                actor_rollout_ref.rollout.name=vllm \
                actor_rollout_ref.rollout.n="${PASS_K}" \
                data.train_files="['gen_results/${MODEL_NAME}/epoch${EPOCH}/deepscaleR_stage2_reward0_correction.parquet']" \
                data.prompt_key=prompt \
                +data.output_path="gen_results/${MODEL_NAME}/epoch${EPOCH}/stage2_correction.parquet"
        ...
        if file_exists_and_nonempty "${STAGE2_EVAL}"; then
            echo "[Step 2.3] Evaluation already done, skipping..."
            echo "Found existing: ${STAGE2_EVAL}"
        else
    ...
    local SFT_DATASET="gen_results/${MODEL_NAME}/epoch${EPOCH}/deepscaleR_stage3_sft.parquet"
    local CKPT_HOME="${CKPT_HOME_BASE}_epoch${EPOCH}"
    ...
    if [[ " ${COMPLETED_STAGES[@]} " =~ " stage3_sft " ]]; then
        ...
        echo "========================================"
        echo "Found existing outputs:"
        echo "  (data): ${STAGE3_SFT_DATA}"
        echo "  (ckpt): ${CKPT_HOME}"
    else
        echo ""
        echo "========================================"
        echo "EPOCH ${EPOCH} - STAGE 3: Policy SFT Training"
        echo "========================================"
        ...
        if ! file_exists_and_nonempty "${STAGE2_DATA}"; then
            echo "ERROR: Stage 2 corrected dataset not found or empty: ${STAGE2_DATA}"
            echo "Please rerun Stage 2 for this epoch before Stage 3."
            exit 1
        fi

        python3 recipe/open_math_reasoning/stage3_prepare.py \
            --stage2_corrected "${STAGE2_DATA}" \
            --output_file "${SFT_DATASET}"

        # Step 3.2: Run SFT training
        echo "[Step 3.2] Running SFT training..."
        mkdir -p "${CKPT_HOME}"

        # Build SFT command
        # Training config: 16K context, LoRA, effective batch size 64
        ...
        SFT_CMD="${SFT_CMD} model.use_remove_padding=true \
            ...
            trainer.experiment_name=\"${MODEL_NAME}_epoch${EPOCH}_sft\" \
    ...
    if [[ " ${COMPLETED_STAGES[@]} " =~ " stage3_eval " ]]; then
        ...
        echo "========================================"
        echo "Found existing evaluation results in: gen_results/${MODEL_NAME}/epoch${EPOCH}/evaluate/"
    else
        ...
        if ! ls "${HF_MERGED_DIR}"/model*.safetensors "${HF_MERGED_DIR}"/pytorch_model*.bin 2>/dev/null | grep -q .; then
            ...
            echo "  2. Delete stage3 SFT marker: rm -f gen_results/${MODEL_NAME}/epoch${EPOCH}/deepscaleR_stage3_sft.parquet"
...
results_file = "results/${MODEL_NAME}/results.json"
...
echo "################################################################################"
echo "# Multi-Epoch Policy Correction Pipeline"
...
for EPOCH in $(seq 1 ${NUM_EPOCHS}); do
    ...
    if [ ${NUM_COMPLETED} -eq 4 ]; then
        ...
        echo "  - gen_results/${MODEL_NAME}/epoch${EPOCH}/"
