#!/bin/bash
# Launcher for the 1-node "share" layout (H200 8-GPU node):
#   srun 1 (teacher):  CUDA_VISIBLE_DEVICES=0          — Qwen3-8B vLLM TP=1
#   srun 2 (ray head): CUDA_VISIBLE_DEVICES=1,2,..,7   — Ray covers actor + rollout
#   srun 3 (driver):   same Ray head, runs main_gkd.py
#
# Ray sees 7 GPUs (after teacher took GPU 0). It places:
#   - actor pool: 4 GPUs (mapped to physical 4,5,6,7)
#   - rollout pool: 3 GPUs (mapped to physical 1,2,3)
# main_gkd.py reads ROLLOUT_GPUS_PER_NODE=3 to size the rollout pool.
set -E
set -o pipefail
...
if [ "${#ALLOCATED_NODES[@]}" -lt 1 ]; then
    echo "ERROR: one_node_share layout requires >= 1 allocated node, got ${#ALLOCATED_NODES[@]}" >&2; exit 1
fi
export SHARED_NODE="${SHARED_NODE:-${ALLOCATED_NODES[0]}}"
export TEACHER_NODE="$SHARED_NODE"
export ROLLOUT_NODE="$SHARED_NODE"
export TRAINING_NODE="$SHARED_NODE"
...
# Combined CUDA_VISIBLE_DEVICES for the Ray cluster = rollout GPUs + actor GPUs
export RAY_CUDA_VISIBLE_DEVICES="${RAY_CUDA_VISIBLE_DEVICES:-${ROLLOUT_CUDA_VISIBLE_DEVICES},${TRAINING_CUDA_VISIBLE_DEVICES}}"
RAY_GPU_COUNT=$((ROLLOUT_GPUS_PER_NODE + TRAINING_GPUS_PER_NODE))
...
echo "=========================================="
echo "OPD Megatron 1-node SHARE layout ($GPU_TYPE x $NODE_GPUS_TOTAL on $SHARED_NODE)"
echo "  job:              ${SLURM_JOB_ID:-manual}"
echo "  teacher:          GPU $TEACHER_CUDA_VISIBLE_DEVICES, TP=$TEACHER_TP_SIZE"
echo "  rollout:          GPU $ROLLOUT_CUDA_VISIBLE_DEVICES, DP=$ROLLOUT_GPUS_PER_NODE"
echo "  actor:            GPU $TRAINING_CUDA_VISIBLE_DEVICES, DP=$TRAINING_GPUS_PER_NODE"
echo "  ray sees:         GPU $RAY_CUDA_VISIBLE_DEVICES ($RAY_GPU_COUNT GPUs)"
echo "  teacher endpoint: $TEACHER_SERVER_HOST:$TEACHER_SERVER_PORT"
echo "  ray head/dashbd:  $SHARED_NODE:$RAY_PORT / $RAY_DASHBOARD_PORT"
echo "  hybrid_engine:    $HYBRID_ENGINE"
echo "=========================================="

# --- 1. Start teacher on GPU 0 ---
echo "Starting teacher on $SHARED_NODE GPU $TEACHER_CUDA_VISIBLE_DEVICES ..."
srun --overlap --nodes=1 --ntasks=1 --nodelist="$SHARED_NODE" --gres="gpu:${GPU_TYPE}:${NODE_GPUS_TOTAL}" \
    /usr/bin/env \
...
# --- 2. Start Ray head covering rollout + actor GPUs on the same node ---
echo "Starting Ray head on $SHARED_NODE covering GPUs $RAY_CUDA_VISIBLE_DEVICES ..."
srun --overlap --nodes=1 --ntasks=1 --nodelist="$SHARED_NODE" --gres="gpu:${GPU_TYPE}:${NODE_GPUS_TOTAL}" \
    /usr/bin/env \
...
# --- 4. Run the training driver on the same node (Ray head) ---
echo "Launching training driver on $SHARED_NODE ..."
srun --overlap --nodes=1 --ntasks=1 --nodelist="$SHARED_NODE" --gres="gpu:${GPU_TYPE}:${NODE_GPUS_TOTAL}" \
    /usr/bin/env \
    CUDA_VISIBLE_DEVICES="$RAY_CUDA_VISIBLE_DEVICES" \
