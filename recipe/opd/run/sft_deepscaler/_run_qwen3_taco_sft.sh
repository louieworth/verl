...
SFT_DATA_TAG="${SFT_DATA_TAG:-taco_canonical_base}"
TRAIN_FILE="${TRAIN_FILE:-$REPO_ROOT/data/train_dataset/taco/canonical/train_sft.parquet}"
LEARNING_RATE="${LEARNING_RATE:-1e-6}"
RUN_ROOT="${RUN_ROOT:-outputs/trd/code/baseline/sft/${MODEL_ALIAS}_${SFT_DATA_TAG}_lr${LEARNING_RATE}}"
...
if [ ! -s "$TRAIN_FILE" ]; then
    echo "ERROR: canonical TACO SFT parquet is missing: $TRAIN_FILE" >&2
    echo "Run recipe/opd/dataset/prepare_experiment_data.py first." >&2
    exit 1
else
    echo "Reusing prepared Base-completion TACO SFT parquet: $TRAIN_FILE"
fi
...
if [ -L "$FINAL_MODEL_LINK" ] && hf_export_complete "$FINAL_MODEL_LINK" && \
   { [ "$STOP_AT_STEP" = "null" ] || [ "$STOP_AT_STEP" = "$TOTAL_TRAINING_STEPS" ]; }; then
    ...
    if [ "$TOTAL_TRAINING_STEPS" = "null" ] || [ "$latest_step" -ge "$TOTAL_TRAINING_STEPS" ]; then
        ...
        echo "Final TACO SFT model: $FINAL_MODEL_LINK -> $current_hf_model"
    else
        echo "TACO SFT milestone model: step $latest_step -> $current_hf_model"
    fi
fi
...
if [ "$RUN_EVAL_AFTER_TRAINING" = "true" ]; then
    ...
    WRITE_RESULTS_CSV=true \
        bash "$REPO_ROOT/recipe/code_evaluation/benchmark_code_model.sh" "$current_hf_model" \
